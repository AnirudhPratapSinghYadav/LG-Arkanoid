...
The phone app (Flutter) is what each player actually interacts with. It's got a drag strip for paddle movement, some power up buttons, and shows your score, lives, and a real-time leaderboard with player names. Players join seamlessly using a phone-driven QR code flow, entering a lobby where everyone waits for the match countdown to finish. It talks to the game server over Socket.io (websockets specifically, for low latency) - sends your paddle position, gets back commentary text and score updates.
